...
def jeda(t):
    for x in range(t + 1):
        print(f'\rTunggu {t} Detik  live={ok}    ', end='')
        sys.stdout.flush()
        t -= 1
        if t == 0:
            break
    ...
    def Kode(self, req1, key, aid):
        ...
        if '"success":true' in str(ps.text):
            ...
            prints(Panel(
                ...
                f'[white][[green]•[white]] Followers           : {self.followers}\n'
                f'[white][[green]•[white]] Following           : {self.following}\n'
            ...
            with open("/sdcard/cokiig.txt", "a") as file:
                file.write(f'{self.cok}\n')
                
            Main2.counter += 1
            
    def fetch_follower_data(self):
        user_url = f"{self.metodes}/{self.user}/?__a=1"
        response = self.r.get(user_url, headers={'User-Agent': self.uas}).json()
        self.followers = response['graphql']['user']['edge_followed_by']['count']  # Fetch followers count
        self.following = response['graphql']['user']['edge_follow']['count']  # Fetch following count